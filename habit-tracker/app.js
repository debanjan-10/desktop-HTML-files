...
(() => {
  ...
  function updateStats() {
    ...
    for (let d = 1; d <= totalDays; d++) {
      ...
      // Streak is the total of fully tracked days (no spaces)
      if (!hasBlank && habits.length > 0) {
        runningStreak++;
      }
    ...
    longestStreak = currentStreak; // Since it doesn't break, longest is current
    ...
    if ($streakState) {
      $streakState.textContent = "";
      $streakState.classList.remove("streak-state--frozen");
    }
    if ($streakBadge) {
      ...
      $streakBadge.setAttribute("aria-label", "Streak active");
    ...
    if ($consistencyScore) {
      const consistency = totalDays ? Math.round((perfectDaysCount / totalDays) * 100) : 0;
  ...
  function openSettings() {
    try { populateHabitsEditor(); } catch(e) { console.error(e); }
    try { renderHistory(); } catch(e) { console.error(e); }
    try { $inputDays.value = totalDays; } catch(e) { console.error(e); }
    try { if ($inputBgPrimary) $inputBgPrimary.value = bgPrimary; } catch(e) { console.error(e); }
    try { if ($inputAccent) $inputAccent.value = accentColor; } catch(e) { console.error(e); }
  ...
  function renderHistory() {
    ...
    const historyRaw = localStorage.getItem("ht_monthly_history");
    const history = historyRaw ? JSON.parse(historyRaw) : [];
    
    if (history.length === 0) {
      $historyList.innerHTML = `<p class="settings-hint">No monthly history saved yet. Click "Save Month" to store your current score.</p>`;
      return;
    }
    ...
    $historyList.innerHTML = history.map(item => `
      <div class="history-item">
        ...
        <span>🔥 ${item.streak} days &nbsp;|&nbsp; 📊 ${item.scorePercentage}%</span>
      </div>
    `).join("");
  }
  ...
  // New function to save the current stats as a month-wise record
  function saveMonthlyStats(monthName) {
    const totalCells = habits.length * totalDays;
    let doneCells = 0;
    let totalFullyTracked = 0;
    ...
    const percentage = totalCells ? Math.round((doneCells / totalCells) * 100) : 0;
    const finalStreak = totalFullyTracked;
    const defaultMonthName = new Date().toLocaleString('default', { month: 'long', year: 'numeric' });

    const monthData = {
      month: monthName || defaultMonthName,
      streak: finalStreak,
    ...
    localStorage.setItem("ht_monthly_history", JSON.stringify(history));

    renderHistory();
    showToast("💾", `Saved ${monthData.month} stats!`);
    console.log("Saved Monthly Stats:", monthData);
    return monthData;
  }
  // Expose it globally so it can be run from the console easily
  window.saveMonthlyStats = saveMonthlyStats;
  ...
  function bindEvents() {
    ...
    if ($btnSaveMonth) $btnSaveMonth.addEventListener("click", () => saveMonthlyStats());
